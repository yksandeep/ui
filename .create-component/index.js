...
import {
  COMPONENT_BASE_DIR_OK,
  createCompo,
  getAllComponentToMakeEvenNested,
  getPathToExportComponent,
  getUserInput,
  logging,
  makeChangesToNewComponent,
  rollback,
} from "./helpers/utilv2.js";
...
(async () => {
  ...
  let { type, ComponentName, cssType } = await getUserInput({
    promptQuestions,
  });
  ...
  if (componentBaseDirOk) {
    ...
    if (origin.length > 0) {
      ...
      let componentCreated = await createCompo({
        ...
        folderToExportFrom,
        BaseDir: COMPONENT_BASE_DIR,
        // defaultStoriesTitle:DEFAULT_STORIES_TITLE,
        // defaultCont:DEFAULT_CONST
      });
      ...
      if (componentCreated) {
        ...
        let madeChanges = await makeChangesToNewComponent(
          Base,
          Component,
        ...
        if (madeChanges) {
          logging("info", "Component Updated");
        } else {
